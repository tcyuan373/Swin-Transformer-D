...
class Graph(abc.ABC):
    ...
    def world(self): # unused for now due to we only have 1 cuda for each node
        ...
        assert self.node_cnt * self._n_cuda_per_process <= len(self._world_list)
...
class Ring(Graph):
    ...
    def get_W(self):
        if self.node_cnt == 1: return torch.ones(1, 1)
...
class Torus(Graph):
    ...
    def get_W(self):
        G = nx.generators.lattice.grid_2d_graph(int(math.sqrt(self.node_cnt)), int(math.sqrt(self.node_cnt)), periodic=True)
...
class SocialNetworkGraph(Graph):
    ...
    def get_W(self):
        ...
        W = torch.from_numpy(nx.adjacency_matrix(graph).toarray().astype(np.float32))
